...
const AddCategoryPage = () => {
    ...
    const handleFormSubmit = (event) => {
        ...
        if (category_name !== '') {
            if (image === null && image_url === '') {
                ...
                if (image !== null) {
                    storageRef.child('categories/'+ guid()).put(image).then(function(snapshot) {
    ...
    return (
        <NavigationDrawer>
            ...
            {isLoading ? <h5>Adding Data..</h5> : <div className="text-center">
                <form onSubmit={handleFormSubmit}>

                    <div style={{marginBottom:"2rem"}}>
                    <TextField id="outlined-basic" label="Category Name" variant="outlined" placeholder="Ex. Apparel" value={category_name} onChange={nameChanged}/>
                    </div>
                    ...
                    <div style={{marginBottom:"2rem"}}>
                        ...
                        {image ? <h5>Selected Image</h5> : 
                        <div>
                        <h5>OR</h5>
                        <TextField id="outlined-basic" label="Image URL" variant="outlined" value={image_url} onChange={urlChanged}/>
                        </div>
                        }
                    </div>
                    <div><Button type= "submit" variant="contained" color="primary">Add Category</Button></div>
                </form>
            </div>}
