...
const FaqDetail = (props) => {
    ...
    const getData = () => {
        db.collection('faq').doc(props.match.params.id)
            .get()
            .then(function(doc) {
                setLoading(false);
    ...
    const handleSubmit = (e) => {
        ...
        db.collection('faq').doc(props.match.params.id).update({
            answer: answer,
            question:  question
        }).then(() => {
            setLoading(false);
    ...
    return (
        <NavigationDrawer>
            {isError ? <h5>ERROR. Something went wrong..!</h5> : (isLoading ? <h5>Loading...</h5> : <form onSubmit={handleSubmit}>
